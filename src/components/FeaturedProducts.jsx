...
export default function FeaturedProducts({ activeTab, setActiveTab, onInquire }) {
    ...
    <section id="products" className="py-20 bg-white border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 mb-12 reveal delay-100">
          ...
          {/* Custom Tabs */}
          <div className="flex flex-wrap gap-2 self-start bg-slate-50 border border-slate-200/50 p-1.5 rounded-[2rem]">
            <button
        ...
        <div className={`grid grid-cols-1 ${packages[activeTab].length === 1 ? '' : 'lg:grid-cols-2'} gap-8`}>
          {packages[activeTab].map((pkg, index) => (
            <div
              key={pkg.id}
              className="glow-card bg-white rounded-3xl border border-slate-100 shadow-sm flex flex-col md:flex-row reveal delay-200 overflow-hidden min-h-[280px]"
